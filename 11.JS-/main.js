...
let front = webTechs2.filter((item) => item === item.toLocaleUpperCase())
console.log(front);
// 5. Reverse the Case
...
function reverseCase(str) {
    ...
    } else (str = str.toLocaleLowerCase()) 
...
//     if (item.uni === 'UNEC' && item.surname[0].toLocaleLowerCase() === 'r') {
//         return item;
//     }
...
//     if (item.age === '22' && item.name[0].toLocaleLowerCase() === 'a') {
//         return item;
//     }
...
let newUser = users.map((item) => {
     if (item.gender.toLocaleLowerCase() === 'male') {
         return item.name;
    } else (item.gender.toLocaleLowerCase() === 'female') 
    return "Mrs " + item.name;
});
//console.log(newUser);
// let byAge =
//users.age.sort(function (a, b) {return a - b});
...
let employee = employees.find((item) => {
    //let max = 0;
    //let emp = employees.skills.length;
    for (let i = 0; i < item.length; i++) {
        let max = 0;
        let emp = item.skills.length;
        if (emp < max)
            max = emp;
        return max;
    }
